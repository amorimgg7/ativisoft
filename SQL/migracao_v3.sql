/* =========================================================
   ...
   Versão: 2026.01.23_v2.6.0
   Objetivo: Unificar tb_colab e tb_cliente em tb_pessoa
             Corrigir todos os vínculos e prevenir conflitos
             usando CPF quando existir e telefone normalizado.
   ========================================================= */
...
VALUES ('2026.01.23_v2.6.0', 'Unificação tb_colab e tb_cliente com normalização de telefone');
...
-- =========================
-- 3. FUNÇÃO PARA NORMALIZAR TELEFONE
-- =========================
DROP FUNCTION IF EXISTS normalize_tel;
DELIMITER //
CREATE FUNCTION normalize_tel(tel VARCHAR(40)) RETURNS VARCHAR(20)
DETERMINISTIC
BEGIN
    DECLARE t VARCHAR(20);
    SET t = REPLACE(REPLACE(REPLACE(REPLACE(tel,' ',''),'(',''),')',''),'-','');
    RETURN t;
END;
//
DELIMITER ;
...
JOIN tb_pessoa p
  ON (c.cpf_colab <> '' AND CONVERT(c.cpf_colab USING latin1) = CONVERT(p.cpf_pessoa USING latin1))
     OR (c.cpf_colab = '' AND normalize_tel(c.tel_colab) = normalize_tel(p.tel1_pessoa));
...
LEFT JOIN tb_pessoa p
  ON (c.cpf_cliente <> '' AND CONVERT(c.cpf_cliente USING latin1) = CONVERT(p.cpf_pessoa USING latin1))
     OR (c.cpf_cliente = '' AND normalize_tel(c.tel_cliente) = normalize_tel(p.tel1_pessoa))
...
-- =========================
-- 7. ADICIONA COLAB_RESP SE NÃO EXISTIR
-- =========================
ALTER TABLE tb_servico
ADD COLUMN IF NOT EXISTS cd_colab_resp INT NULL;

-- =========================
-- 8. CORREÇÃO DE VÍNCULOS
...
LEFT JOIN tb_pessoa p1 ON p1.cd_pessoa = mf.cd_colab_movimento
LEFT JOIN tb_pessoa p2 ON p2.cd_pessoa = mf.cd_cliente_movimento
WHERE p1.cd_pessoa IS NULL OR p2.cd_pessoa IS NULL;

COMMIT;

/* =========================================================
   FIM DA MIGRAÇÃO COMPLETA v2.6.0
   ========================================================= */
...
-- =========================
-- Atualiza cd_cliente na tb_servico usando telefone
-- =========================
UPDATE tb_servico s
JOIN tb_cliente_old c_old ON c_old.cd_cliente = s.cd_cliente
JOIN tb_pessoa p_new 
    ON normalize_tel(c_old.tel_cliente) = normalize_tel(p_new.tel1_pessoa)
SET s.cd_cliente = p_new.cd_pessoa
WHERE s.cd_cliente IS NOT NULL;


-- Orçamentos
UPDATE tb_orcamento_servico os
JOIN tb_cliente_old c_old ON c_old.cd_cliente = os.cd_cliente
JOIN tb_pessoa p_new 
    ON normalize_tel(c_old.tel_cliente) = normalize_tel(p_new.tel1_pessoa)
SET os.cd_cliente = p_new.cd_pessoa
WHERE os.cd_cliente IS NOT NULL;

-- Movimentos financeiros
UPDATE tb_movimento_financeiro mf
JOIN tb_cliente_old c_old ON c_old.cd_cliente = mf.cd_cliente_movimento
JOIN tb_pessoa p_new 
    ON normalize_tel(c_old.tel_cliente) = normalize_tel(p_new.tel1_pessoa)
SET mf.cd_cliente_movimento = p_new.cd_pessoa
WHERE mf.cd_cliente_movimento IS NOT NULL;

-- Contratos
-- UPDATE tb_contrato ct
-- JOIN tb_cliente_old c_old ON c_old.cd_cliente = ct.cd_contratante
-- JOIN tb_pessoa p_new 
--     ON normalize_tel(c_old.tel_cliente) = normalize_tel(p_new.tel1_pessoa)
-- SET ct.cd_contratante = p_new.cd_pessoa
