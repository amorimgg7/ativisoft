...
$_SESSION['fromEncoding'] = 'UTF-8';
        ...
        //require_once $_SESSION['dominio'].'/classes/conn.php';
        //include($_SESSION['dominio']."/classes/functions.php");
...
function customHeader($modelo, $text, $os) {
	if($modelo == 'M1'){
		$this->SetFont('Arial', 'B', 20);
        $this->Cell(80, 10, mb_convert_encoding($text, $_SESSION['toEncoding'], $_SESSION['fromEncoding']), 0, 1, 'C');
        $this->Ln(5);
		$this->SetFont('Arial', 'B', 20);
		$this->Cell(80, 7, mb_convert_encoding($text, $_SESSION['toEncoding'], $_SESSION['fromEncoding']).': '.$os, 1, 1, 'C');
		$this->Ln(1);
	}elseif($modelo == 'M2'){
		$this->SetFont('Arial', 'B', 20);
        $this->Cell(80, 10, mb_convert_encoding($text, $_SESSION['toEncoding'], $_SESSION['fromEncoding']), 0, 1, 'C');
        ...
        $servico = "Serviço \n ".$os."";
        $this->MultiCell($larguraCelula, 10, mb_convert_encoding($servico, $_SESSION['toEncoding'], $_SESSION['fromEncoding']), 1, 'C');
            
	}
}
function customFooter($modelo, $text, $assina = true) {
	if($modelo == 'M1'){
		if($text != ''){
			$this->SetFont('Arial', 'B', 10);

			$this->Cell(0, 2, '', 0, 1, 'C');
			$this->MultiCell(80, 7, $this->WrapText(mb_convert_encoding($text, $_SESSION['toEncoding'], $_SESSION['fromEncoding'])), 1, 'C');
			$this->Cell(0, 0, '', 0, 1, 'C');
        	$this->Ln(5);
		}
		if($assina){
			$this->SetFont('Arial', 'B', 10);
			$this->MultiCell(80, 7, $this->WrapText(mb_convert_encoding('Ass. Cliente: __________________', $_SESSION['toEncoding'], $_SESSION['fromEncoding'])), 0, 'C');
			$this->MultiCell(80, 7, $this->WrapText(mb_convert_encoding('Ass. Funcionario: __________________', $_SESSION['toEncoding'], $_SESSION['fromEncoding'])), 0, 'C');
		}
...
function dadosCliente($modelo, $nomeCompleto, $cpf_cnpj, $telefone, $obs) {
	if($modelo == 'M1'){
        ...
        $obs = 'OBS: '.mb_convert_encoding($this->WrapText($obs), $_SESSION['toEncoding'], $_SESSION['fromEncoding']);
        $h_obs = $this->NbLines($w_obs, $obs) * 5;
        // Maior altura da linha
        $h_linha = $h_obs;
        // Coluna 1: contador
        $this->SetXY($x, $y);
        // Coluna 2: título (MultiCell com quebra de linha)
        $this->MultiCell($w_obs, 5, $obs, 1, 'C');
        $this->Ln(1);
	}elseif($modelo == 'M2'){
		$this->SetFont('Arial', 'B', 15);
        $this->MultiCell(80, 7, 'Cliente', 1, 'C');
        $this->SetFont('Arial', 'B', 15);
        $this->Cell(80, 7, 'Nome: '.mb_convert_encoding($nomeCompleto,$_SESSION['toEncoding'], $_SESSION['fromEncoding']), 1, 1, 'L');
        $this->Cell(80, 7, 'Telefone: +'.$telefone, 1, 1, 'L');
		//(W, H, TEXT), 
        $w_obs  = 80;
        // Fonte
        $this->SetFont('Arial', 'B', 15);
...
function dadosEmpresa($modelo, $nfantasia_filial, $cnpj_filial, $endereco_filial) {
	if($modelo == 'M1'){
		$this->SetFont('Arial', 'B', 15);
		$this->Cell(80, 5, "-", 0, 1, 'C');
		$this->Cell(80, 5, mb_convert_encoding($nfantasia_filial, $_SESSION['toEncoding'], $_SESSION['fromEncoding']), 0, 1, 'C');
		$this->SetFont('Arial', 'B', 10);
		$this->Cell(80, 5, 'CNPJ: '.$cnpj_filial, 0, 1, 'C');
		$this->MultiCell(80, 5, mb_convert_encoding($this->WrapText($endereco_filial), $_SESSION['toEncoding'], $_SESSION['fromEncoding']), 0, 'C');
		$this->Ln(1);
	}elseif($modelo == 'M2'){
		$this->SetFont('Arial', 'B', 15);
		$this->Cell(80, 5, "-", 0, 1, 'C');
		...
		$this->SetFont('Arial', 'B', 14);
		$this->Cell(0, 8, mb_convert_encoding('Prestador de serviços', $_SESSION['toEncoding'], $_SESSION['fromEncoding']), 1, 1, 'L');
		$this->SetFont('Arial', '', 10);

//mb_convert_encoding($cliente, $_SESSION['toEncoding'], $_SESSION['fromEncoding'])
		

		$numeros_cnpj_filial = preg_replace('/\D/', '', $cnpj_filial);

		if (strlen($numeros_cnpj_filial) == 11) {//05185255544
			$cliente = "Nome: $nfantasia_filial\nCPF: $cnpj_filial";
		}elseif(strlen($numeros_cnpj_filial) == 14){//02077969000100
			$cliente = "Nome Fantasia: $nfantasia_filial\nCNPJ: $cnpj_filial\nEndereço: $endereco_filial";
		}else{
			$cliente = "Dados do prestador de serviço faltando.";
		}
...
function detalheServico($modelo, $prioridade, $previsao, $os) {

	if($prioridade == "U"){
        $ds_prioridade = 'Urgente';
    }
    if($prioridade == "A"){
        $ds_prioridade = 'Alta';
    }
	...
	if($modelo == 'M1'){
		...
		$this->SetFont('Arial', 'B', 15);
        $this->MultiCell(80, 7, mb_convert_encoding('Detalhes do Serviço', $_SESSION['toEncoding'], $_SESSION['fromEncoding']), 1, 'C');
        $this->Cell(80, 7, 'Prioridade: '.$ds_prioridade, 1, 1, 'L');
        $this->Cell(80, 7, mb_convert_encoding('Previsão: '.date('d/m/Y \a\s H:i', strtotime($previsao)), $_SESSION['toEncoding'], $_SESSION['fromEncoding']), 1, 1, 'L');
        $this->SetFont('Arial', 'B', 10);
        $this->Ln(1);
        //session_start();
        require_once '../../classes/conn.php';
        ...
        $this->SetFont('Arial', 'B', 15);
        $this->Cell(5, 7, '#', 1, 0, 'C');
        $this->Cell(50, 7, mb_convert_encoding('Descrição', $_SESSION['toEncoding'], $_SESSION['fromEncoding']), 1, 0, 'C');
        $this->Cell(25, 7, 'Valor', 1, 1, 'C'); // último parâmetro 1 = quebra linha
        $select_orcamento = "SELECT * FROM tb_orcamento_servico WHERE cd_servico = '".$os."' ORDER BY cd_orcamento ASC";
        $result_orcamento = mysqli_query($conn, $select_orcamento);
        $count = 0;
        while($row_orcamento = $result_orcamento->fetch_assoc()) {
        ...
        include("../../classes/functions.php");
        $this->SetFont('Arial', 'B', 14);
        $this->MultiCell(0, 7, 'Lista detalhada', 1, 'L');
        $this->SetFont('Arial', 'B', 14);
        $this->Cell(5, 7, '#', 1, 0, 'C');
        $this->Cell(150, 7, mb_convert_encoding('Descrição', $_SESSION['toEncoding'], $_SESSION['fromEncoding']), 1, 0, 'C');
        $this->Cell(35, 7, 'Valor', 1, 1, 'C'); // último parâmetro 1 = quebra linha
        $select_orcamento = "SELECT * FROM tb_orcamento_servico WHERE cd_servico = '".$os."' ORDER BY cd_orcamento ASC";
        $result_orcamento = mysqli_query($conn, $select_orcamento);
        $count = 0;
		...
		//$this->SetFont('Arial', '', 10);
		//$cliente = "Cliente: $nomeCompleto\nTelefone: +$telefone\nEndereço: asd,123";
		//$this->MultiCell(0, 10, mb_convert_encoding($cliente, $_SESSION['toEncoding'], $_SESSION['fromEncoding']), 1);
		//$this->SetFont('Arial', '', 10);
		//$obs_formatado = 'OBS: ' . mb_convert_encoding($this->WrapText($obs), $_SESSION['toEncoding'], $_SESSION['fromEncoding']);
		//$this->MultiCell(0, 10, $obs_formatado, 1, 'L');
...
function detalheVenda($modelo, $venda) {

	if($modelo == 'M1'){
		$this->SetFont('Arial', 'B', 15);
        $this->MultiCell(80, 7, mb_convert_encoding('Detalhes da Venda', $_SESSION['toEncoding'], $_SESSION['fromEncoding']), 1, 'C');
        $this->SetFont('Arial', 'B', 10);
        $this->Ln(1);
        //session_start();
        require_once '../classes/conn.php';
        ...
        $this->Cell(40, 7, mb_convert_encoding('Descrição', $_SESSION['toEncoding'], $_SESSION['fromEncoding']), 1, 0, 'C');
        $this->Cell(35, 7, 'Valor', 1, 1, 'C'); // último parâmetro 1 = quebra linha
        $select_orcamento = "SELECT * FROM tb_orcamento_venda WHERE cd_venda = '".$venda."' ORDER BY cd_orcamento ASC";
        $result_orcamento = mysqli_query($conn, $select_orcamento);
        $count = 0;
        while($row_orcamento = $result_orcamento->fetch_assoc()) {
            $count++;
		...
		$complementares = "Prioridade: \nPrevisão de Entrega/Finalização: ";
...
function detalheRetrabalhoServico($modelo, $prioridade, $obs_retrabalho, $dt_inicio_retrabalho, $dt_prazo_retrabalho, $os) {

	if($modelo == 'M1'){
		$this->SetFont('Arial', 'B', 15);
        $this->MultiCell(80, 7, mb_convert_encoding('Informações da Garantia', $_SESSION['toEncoding'], $_SESSION['fromEncoding']), 1, 'C');
        $this->Cell(40, 7, 'Prioridade', 1, 0, 'C');
        $this->Cell(40, 7, mb_convert_encoding('Previsão', $_SESSION['toEncoding'], $_SESSION['fromEncoding']), 1, 1, 'C');
        $this->SetFont('Arial', 'B', 10);
        if($prioridade == "U"){
        ...
        $this->Cell(40, 7, date('d/m/Y \a\s H:i', strtotime($dt_prazo_retrabalho)), 1, 1, 'C');
        $this->Ln(1);
        //session_start();
        ...
        $this->MultiCell(80, 7, mb_convert_encoding('Informações da Garantia', $_SESSION['toEncoding'], $_SESSION['fromEncoding']), 1, 'C');
        $this->Cell(80, 7, 'Prioridade: '.$ds_prioridade, 1, 1, 'L');
        $this->Cell(80, 7, mb_convert_encoding('Início: '.date('d/m/Y \a\s H:i', strtotime($dt_inicio_retrabalho)), $_SESSION['toEncoding'], $_SESSION['fromEncoding']), 1, 1, 'L');
        $this->Cell(80, 7, mb_convert_encoding('Previsão: '.date('d/m/Y \a\s H:i', strtotime($dt_prazo_retrabalho)), $_SESSION['toEncoding'], $_SESSION['fromEncoding']), 1, 1, 'L');
		$this->MultiCell(0, 8, mb_convert_encoding('OBS: '.$obs_retrabalho, $_SESSION['toEncoding'], $_SESSION['fromEncoding']), 1);
		...
		$previsao = date('d/m/Y \a\s H:i', strtotime(''));
		$complementares = "Prioridade: $prioridadee\nPrevisão de Entrega/Finalização: $previsao";
...
function dadosFinanceiros($os, $modelo, $vorcamento, $vpago) {
	if($modelo == 'M1'){
		$faltapagar = floatval($vorcamento) - floatval($vpago);

                $this->SetFont('Arial', 'B', 15);
                $this->Cell(80, 7, 'Financeiro', 1, 1, 'C');
                
                if($faltapagar == 0){
                    $this->SetFont('Arial', 'B', 10);
                    $this->Cell(80, 7, 'Total Pago: ' . $vpago, 1, 1, 'C');
                }else{
                    $this->SetFont('Arial', 'B', 10);
                    $this->Cell(25, 7, mb_convert_encoding('Orçamento', $_SESSION['toEncoding'], $_SESSION['fromEncoding']), 1, 0, 'C');
                    $this->Cell(30, 7, mb_convert_encoding('Pago', $_SESSION['toEncoding'], $_SESSION['fromEncoding']), 1, 0, 'C');
                    $this->Cell(25, 7, mb_convert_encoding('Falta', $_SESSION['toEncoding'], $_SESSION['fromEncoding']), 1, 1, 'C');
                    ...
                    $this->Cell(25, 7, $faltapagar, 1, 1, 'L');
                    $this->Ln(1);
                }
	}elseif($modelo == 'M2'){
		$faltapagar = floatval($vorcamento) - floatval($vpago);
		...
		// Título da seção
		$this->SetFont('Arial', 'B', 14);
        $this->Cell(0, 8, mb_convert_encoding('Financeiro', $_SESSION['toEncoding'], $_SESSION['fromEncoding']), 1, 'L');
        $this->SetFont('Arial', '', 10);
        $complementares = '';
		if($vorcamento > $vpago){
			$complementares = $complementares.'Falta pagar R$:'.number_format($vorcamento-$vpago, 2, '.');	
		}elseif($vorcamento == $vpago){
			$complementares = $complementares.'Total pago R$:'.number_format($vpago, 2, '.');	
		}else{
			$complementares = $complementares.'? R$:'.number_format($vpago, 2, '.');	
		}
		//$complementares = "OS: $os\nOrcamento: $vorcamento\nPago: $vpago\nObservações sobre dados financeiros! \n($modelo)";
		
        //require_once '../../classes/conn.php';
        //include("../../classes/functions.php");
        //$select_movimento = "SELECT * FROM tb_movimento_financeiro WHERE cd_os_movimento = 5 ORDER BY cd_movimento ASC";
        //$result_movimento = mysqli_query($conn, $select_movimento);
        //$count = 0;
		//$total_servico = 0;
        /*while($row_movimento = $result_movimento->fetch_assoc()) {
            $count++;
            // Define as larguras
            $w_count   = 5;
            $w_titulo  = 150;
...
function dadosFinanceirosVenda($venda, $modelo, $vorcamento, $vpago) {
	if($modelo == 'M1'){
		$faltapagar = floatval($vorcamento) - floatval($vpago);
...
function detalhesAtividadesServico($modelo, $os) {
	if($modelo == 'M1'){
        ...
        while($row_servico = $result_servico->fetch_assoc()) {
			...
			$this->MultiCell(0, 7, mb_convert_encoding('OBS: '.$row_servico['obs_atividade'], $_SESSION['toEncoding'], $_SESSION['fromEncoding']), 1);
        ...
        while($row_lastservico = $result_lastservico->fetch_assoc()) {
            ...
            if(isset($row_lastservico['fim_atividade'])){
				...
				$this->MultiCell(0, 7, mb_convert_encoding('OBS: '.$row_lastservico['obs_atividade'], $_SESSION['toEncoding'], $_SESSION['fromEncoding']), 1);
		...
		$this->SetFont('Arial', 'B', 15);
    	$this->MultiCell(80, 7, mb_convert_encoding('Histórico Simples', $_SESSION['toEncoding'], $_SESSION['fromEncoding']), 1, 'C');

		$this->Cell(80, 7, mb_convert_encoding('Detalhes da Atividade', $_SESSION['toEncoding'], $_SESSION['fromEncoding']), 1, 1, 'C');
