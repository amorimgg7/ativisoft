    ...
    if (isset($_POST['imprimir_os'])) {
        ...
        class MeuPDF extends FPDF {
            ...
            function GerarOrdemServico($showcd_servico, $nome, $sobrenome, $telefone, $showtitulo_servico, $showobs_servico, $showprioridade_servico, $showprazo_servico, $showorcamento_servico, $showvpag_servico) {
                ...
                $this->dadosFinanceiros($showcd_servico, 'M1', $showorcamento_servico, $showvpag_servico);
        ...
        $sobrenome = $_POST['btnsnome_cliente'];
        if(!isset($_POST['btnvcusto_orcamento'])){
            $showorcamento_venda = 0;
        }else{
            $showorcamento_venda = $_POST['btnvcusto_orcamento'];
        }
        $showvpag_venda = $_POST['btnvpag_orcamento'];
        ...
        class MeuPDF extends FPDF {
            ...
            function GerarImpressaoVenda($showcd_venda, $nome, $sobrenome, $showorcamento_venda, $showvpag_venda) {
                $this->AddPage('P', array(80, 1000)); // Tamanho do papel em milímetros
                $this->SetAutoPageBreak(false);
                $this->SetLeftMargin(0);
                $this->SetFont('Arial', 'B', 20);
                ...
                $this->customHeader('M1', 'Venda', $showcd_venda, );

                //$this->dadosCliente('M1', $nome.$sobrenome,'','', '');
                
                $this->detalheVenda('M1', $showcd_venda);
                
                $this->dadosFinanceiros($showcd_venda, 'M1', $showorcamento_venda, $showvpag_venda);
                
                //$this->customFooter('M1', '', false);
        ...
        // Instanciar a classe MeuPDF e gerar o PDF
        $pdf = new MeuPDF();
        $pdf->GerarImpressaoVenda($showcd_venda, $nome, $sobrenome, $showorcamento_venda, $showvpag_venda);
