...
function listaCaixas($modelo, $inicio, $fim, $ocorrencias = null) {
	/*if($modelo == 'M1'){
...
while ($row_caixa = $result_caixa->fetch_assoc()) {
	...
	if($ocorrencias == 'S'){
		...
		$select_movimento_financeiro = "SELECT * FROM tb_movimento_financeiro WHERE cd_caixa_movimento = '".$row_caixa['cd_caixa']."' order by tipo_movimento asc;";
		$result_movimento_financeiro = mysqli_query($conn, $select_movimento_financeiro);
		$count_movimento_financeiro = 0;
		$count_receita = 0;
		$count_despesa = 0;
		$count_comissao = 0;
		while ($row_movimento_financeiro = $result_movimento_financeiro->fetch_assoc()) {
			$count_movimento_financeiro++;
			$w_count = 10;
			$w_caixa = 130;
			$w_valor = 50;

			if($row_movimento_financeiro['tipo_movimento'] == '1' || $row_movimento_financeiro['tipo_movimento'] == '2'){
				$count_receita++;
				if($count_receita == 1){$caixa .= "\n\nReceitas(Entradas)";}
				$caixa .= "\n$count_movimento_financeiro - (".$row_movimento_financeiro['fpag_movimento']." - ".$row_movimento_financeiro['valor_movimento']." - ".$row_movimento_financeiro['obs_movimento'].")";
			}else if($row_movimento_financeiro['tipo_movimento'] == '3'){
				$count_despesa++;
				if($count_despesa == 1){$caixa .= "\n\nDespesas(Saídas)";}
				$caixa .= "\n$count_movimento_financeiro - (".$row_movimento_financeiro['fpag_movimento']." - ".$row_movimento_financeiro['valor_movimento']." - ".$row_movimento_financeiro['obs_movimento'].")";
			}else{
				$caixa .= "\n0 - (".$row_movimento_financeiro['fpag_movimento']." - ".$row_movimento_financeiro['valor_movimento']." - ".$row_movimento_financeiro['obs_movimento'].")";
			}
		}		
	...
	}else if($ocorrencias == 'N'){
		
	}else{
		$caixa .= "flag fl_ocorrencia Desconhecido";
	}
	$x = $this->GetX();
	$y = $this->GetY();
