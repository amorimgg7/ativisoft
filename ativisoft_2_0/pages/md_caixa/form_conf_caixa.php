                        ...
                        <div class="col-sm-12 mb-4 mb-xl-0">
                            <?php
                                if(isset($_POST['con_conf_cd_caixa']))
                                {
                                    ...
                                    $select_caixa_conferido = "SELECT * FROM tb_caixa WHERE cd_filial = ".$_SESSION['cd_empresa']." AND cd_caixa = '".$_POST['con_conf_cd_caixa']."'";
                                    //echo '<h3>'.$dia_hoje.'</h3>';
                                    ...
                                    if ($resulta_caixa_conferido->num_rows > 0){ 
                                        ...
                                        while ( $row_caixa_conferido = $resulta_caixa_conferido->fetch_assoc()){
                                            ...
                                            $select_responsavel_conferido = "SELECT * FROM tb_pessoa WHERE cd_pessoa = '".$row_caixa_conferido['cd_colab_abertura']."'";
                                            //insert into tb_caixa(dt_abertura, cd_colab_abertura, saldo_abertura, status_caixa) VALUES('2023-08-12T13:00','1','15','0')
                                            ...
                                            while ($row_responsavel_conferido = $resulta_responsavel_conferido->fetch_assoc()){
                                                echo '<td>'.$row_responsavel_conferido['pnome_pessoa'].' '.$row_responsavel_conferido['snome_pessoa'].'</td>';
                                            }
                                        ...
                                        $select_caixa_conferido2 = "SELECT * FROM tb_caixa_conferido WHERE cd_filial = ".$_SESSION['cd_empresa']." AND cd_caixa_analitico = '".$_POST['con_conf_cd_caixa']."'";
                                        ...
                                        if ($resulta_caixa_conferido2->num_rows > 0){ 
                                            ...
                                            while ( $row_caixa_conferido2 = $resulta_caixa_conferido2->fetch_assoc()){
                                                ...
                                                //echo '<td>'.$row_caixa['dt_abertura'].'/'.$row_caixa['dt_fechamento'].'</td>';
                                                $select_responsavel_conferido = "SELECT * FROM tb_pessoa WHERE cd_pessoa = '".$row_caixa_conferido2['cd_colab_conferencia']."'";
                                                //insert into tb_caixa(dt_abertura, cd_colab_abertura, saldo_abertura, status_caixa) VALUES('2023-08-12T13:00','1','15','0')
                                                $resulta_responsavel_conferido = $conn->query($select_responsavel_conferido);
                                                while ($row_responsavel_conferido = $resulta_responsavel_conferido->fetch_assoc()){
                                                    echo '<td>'.$row_responsavel_conferido['pnome_pessoa'].' '.$row_responsavel_conferido['snome_pessoa'].'</td>';
								...
								$select_movimento_grafico = "SELECT * FROM tb_movimento_financeiro WHERE cd_filial = ".$_SESSION['cd_empresa']." AND cd_caixa = '".$_SESSION['cd_caixa_conferido']."'";
                                $resulta_caixa_conferido = $conn->query($select_caixa_conferido);
                                if ($resulta_caixa_conferido->num_rows > 0)
								{ 
									while ( $row_caixa_conferido = $resulta_caixa_conferido->fetch_assoc())
                                ...
                                if (isset($_POST['con_conf_cd_caixa'])) {
                                    ...
                                    $query_fpag_debito = "SELECT SUM(valor_movimento) as soma_debito 
                                        FROM tb_movimento_financeiro 
                                        WHERE cd_caixa_movimento = '".$_SESSION['cd_caixa_conferido']."' 
                                        AND fpag_movimento = 'debito'  AND tipo_movimento = 1";

                                    $query_fpag_credito = "SELECT SUM(valor_movimento) as soma_credito 
                                        FROM tb_movimento_financeiro 
                                        WHERE cd_caixa_movimento = '".$_SESSION['cd_caixa_conferido']."' 
                                        AND fpag_movimento = 'credito' AND tipo_movimento = 1";

                                    $query_fpag_pix = "SELECT SUM(valor_movimento) as soma_pix 
                                        FROM tb_movimento_financeiro 
                                        WHERE cd_caixa_movimento = '".$_SESSION['cd_caixa_conferido']."' 
                                        AND fpag_movimento = 'pix' AND tipo_movimento = 1";

                                    $query_fpag_cofre = "SELECT SUM(valor_movimento) as soma_cofre 
                                        FROM tb_movimento_financeiro 
                                        WHERE cd_caixa_movimento = '".$_SESSION['cd_caixa_conferido']."' 
                                        AND fpag_movimento = 'cofre' AND tipo_movimento = 1";
								...
								<div class="card">
									<div class="card-body">
                                        <?php
                                            ...
                                            echo '</thead>';
                                            echo '<tbody>';
                                            $select_movimento_caixa = "SELECT * FROM tb_movimento_financeiro WHERE cd_filial = ".$_SESSION['cd_empresa']." AND cd_caixa_movimento = '".$_SESSION['cd_caixa_conferido']."'";
                                            //insert into tb_caixa(dt_abertura, cd_colab_abertura, saldo_abertura, status_caixa) VALUES('2023-08-12T13:00','1','15','0')
...
<!-- NUMERO OS -->
<div class="input-group mb-2" id="os_movimento" style="display:none;">
    <div class="input-group-prepend">
        <span class="input-group-text">OS</span>
    </div>
    <input type="text" class="form-control form-control-sm" name="os_mov" placeholder="Número da OS">
</div>


<!-- FORMA PAGAMENTO -->
<div class="input-group mb-2" id="grupo_fpag" style="display:none;">
    <div class="input-group-prepend">
        <span class="input-group-text">#</span>
    </div>

    <select id="fpag_movimento" name="fpag_movimento" class="form-control form-control-sm">
        <option value=""></option>
...
<!-- VALOR -->
<div class="input-group mb-2" id="grupo_valor" style="display:none;">
    <div class="input-group-prepend">
        <span class="input-group-text">R$</span>
    </div>
    <input id="vpag_movimento" name="vpag_movimento"
           type="tel"
           class="form-control form-control-sm"
           placeholder="Valor">
</div>
...
<!-- MOTIVO -->
<div class="input-group mb-2" id="grupo_motivo" style="display:none;">
    <div class="input-group-prepend">
        <span class="input-group-text">Motivo</span>
    </div>
    <textarea class="form-control" id="motivo_movimento" name="motivo_movimento" rows="3"></textarea>

</div>

<button type="submit" class="btn btn-lg btn-block btn-outline-success" id="con_OS" name="con_OS" style="display:none;"><i class="mdi mdi-file-check"></i>Consuta OS</button>
...
function selectTipo() {
    ...
    const os      = document.getElementById("os_movimento");
    const fpag    = document.getElementById("grupo_fpag");
    const valor   = document.getElementById("grupo_valor");
    const motivo  = document.getElementById("grupo_motivo");
    const motivoSelect = document.getElementById("motivo_movimento");
    const btnConOS = document.getElementById("con_OS");

    console.log("Tipo selecionado:", tipo);

    // RESET
    os.style.display = "none";
    fpag.style.display = "none";
    valor.style.display = "none";
    motivo.style.display = "none";
    btnConOS.style.display = "none";
    motivoSelect.innerHTML = '';

    // PAGAMENTO OS
    if (tipo === "1") {
        os.style.display = "flex";
        fpag.style.display = "flex";
        valor.style.display = "flex";
        motivo.style.display = "flex";
        btnConOS.style.display = "flex";
