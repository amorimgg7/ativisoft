...
if (!is_dir($diretorioCertificado)) {

    mkdir($diretorioCertificado, 0755, true);

}

/*
|--------------------------------------------------------------------------
| NOME PADRÃO CERTIFICADO
|--------------------------------------------------------------------------
*/
...
|--------------------------------------------------------------------------
*/
if (isset($_POST['excluir_certificado'])) {

    if (file_exists($arquivoCertificado)) {

        if (unlink($arquivoCertificado)) {
            ...
            unset($_SESSION['senha_certificado']);

            echo "
            <script>
                window.alert('Certificado removido com sucesso!');
            ...
            echo "
            <script>
                window.alert('Não foi possível remover o certificado!');
            </script>
            ";
        ...
        echo "
        <script>
            window.alert('Certificado não encontrado!');
        </script>
        ";
...
if (isset($_POST['gravaFiscal_Funcao'])) {
    ...
    |--------------------------------------------------------------------------
    */
    $iestadual_empresa = mysqli_real_escape_string(
        $conn,
        $_POST['iestadual_empresa']
    );

    $regime_fiscal = mysqli_real_escape_string(
        $conn,
        $_POST['regime_fiscal']
    );

    $ambiente_fiscal = mysqli_real_escape_string(
        $conn,
        $_POST['ambiente_fiscal']
    );

    $csc_nfce = mysqli_real_escape_string(
        $conn,
        $_POST['csc_nfce']
    );

    $id_csc_nfce = mysqli_real_escape_string(
        $conn,
        $_POST['id_csc_nfce']
    );

    $cnae_fiscal = mysqli_real_escape_string(
        $conn,
        $_POST['cnae_fiscal']
    );

    $imunicipal_empresa = mysqli_real_escape_string(
        $conn,
        $_POST['imunicipal_empresa']
    );

    $senha_certificado = mysqli_real_escape_string(
        $conn,
    ...
    */
    if (
        isset($_FILES['certificado_digital']) &&
        $_FILES['certificado_digital']['error'] == 0
    ) {

        $ext = strtolower(
            pathinfo(
                $_FILES['certificado_digital']['name'],
                PATHINFO_EXTENSION
            )
        );
        ...
        if ($ext == 'pfx' || $ext == 'p12') {
            ...
            */
            if (
                move_uploaded_file(
                    $_FILES['certificado_digital']['tmp_name'],
                    $arquivoCertificado
                )
            ) {
                ...
                echo "
                <script>
                    window.alert('Certificado enviado com sucesso!');
                </script>
                ";
                ...
                echo "
                <script>
                    window.alert('Erro ao enviar certificado!');
                </script>
                ";
            ...
            echo "
            <script>
                window.alert('Arquivo inválido! Envie apenas .PFX ou .P12');
            </script>
            ";
    ...
    */
    ...
    if (mysqli_query($conn, $query)) {
        ...
        echo "
        <script>
            window.alert('Configurações fiscais atualizadas com sucesso!');
        </script>
        ";
        ...
        echo "
        <script>
            window.alert('Erro ao atualizar configurações fiscais!');
        </script>
        ";
