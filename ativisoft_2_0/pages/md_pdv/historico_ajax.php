    ...
    <div class="accordion-item">
        <h2 class="accordion-header" id="heading<?= $cd_venda ?>">
            <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse"
                data-bs-target="#collapse<?= $cd_venda ?>" aria-expanded="false"
                aria-controls="collapse<?= $cd_venda ?>">
                Venda #<?= $cd_venda ?> - <?= htmlspecialchars($cliente) ?> - <?= $abertura ?> - Total: R$ <?= $total ?>
            </button>
        </h2>
        <div id="collapse<?= $cd_venda ?>" class="accordion-collapse collapse" aria-labelledby="heading<?= $cd_venda ?>"
                    ...
                    <tbody>
                        <?php while ($item = $resItens->fetch_assoc()) { ?>
                            <tr>
                                ...
                                <td class="text-end"><?= number_format($item['vtotal_orcamento'], 2, ',', '.') ?></td>
                            </tr>
                        <?php } ?>
                    </tbody>
                </table>
                <p><strong>Subtotal:</strong> R$ <?= $subtotal ?> <br>
