...
if(isset($_POST['pagar'])){
  $retorno = $f->movimentoFinanceiro(
    ...
    number_format($_POST['vpag_movimento'], 2, '.', '.')
  );
...
/* ===== PERMISSÕES ===== */
$permAddProduto = $u->retPermissaoBool('813');
$permDesconto   = $u->retPermissaoBool('814');
$permRemover    = $u->retPermissaoBool('810');
      ...
      <div class="card-body">
        ...
        <?php if ($permAddProduto): ?>
          <form method="post" action="0_add.php">
            ...
            <input type="hidden" name="estoque" id="estoque_<?= $p['id'] ?>" value="<?= $p['estoque'] === null ? 'null' : $p['estoque'] ?>">

            <button id="btn_add_<?= $p['id'] ?>" class="btn btn-sm btn-primary w-100" type="submit">Adicionar</button>

            <small class="text-muted">
              <?= $p['estoque'] === null ? 'Não controla estoque' : 'Estoque disponível: '.htmlspecialchars($p['estoque']) ?>
            </small>
          </form>
          ...
          <script>
            (function () {
              const estoqueEl = document.getElementById("estoque_<?= $p['id'] ?>");
              const btn = document.getElementById("btn_add_<?= $p['id'] ?>");

              // estoque vazio = NULL (não controla estoque)
              if (estoqueEl.value === '') {
                return; // libera normalmente
              }

              const estoque = parseInt(estoqueEl.value, 10);
    ...
    <div class="col-md-3">
      <div class="card">
        ...
        <div class="col-md-12">
          ...
          <?php
            if(is_null($_SESSION['cd_cliente_padrao'])){
              $retClientes = $u->conPessoa('cliente', 'lista');
              if($retClientes['status'] != 'OK'){
                echo "<script>alert('".$retClientes['status']."');</script>";
              }else{
                echo $retClientes['partial_cliente'];
              }
            }else{
              $retClientes = $u->conPessoa('cliente', 'codigo', $_SESSION['cd_cliente_padrao']);
              if($retClientes['status'] != 'OK'){
                echo "<script>alert('".$retClientes['status']."');</script>";
              }else{
                $_SESSION['cd_cliente_venda'] = $retClientes['cd_cliente'];
                echo '<label class="form-label small">Cliente Padrão: </label>';
                echo '('.$retClientes['cd_cliente'].')'.$retClientes['pnome_cliente'].' '.$retClientes['snome_cliente'];
              }
            }
          ?>  
          ...
          <?php
          if(isset($_SESSION['cd_venda']) && $_SESSION['cd_venda'] > 0){
            echo '<h5>Carrinho.ID ('.$_SESSION['cd_venda'].')</h5>';
            //echo '<h5>Carrinho.Venda aberta: '.$_SESSION['cd_venda'].'</h5>';
            //echo '<h5>Venda Sequencial: '.$_SESSION['cd_venda_seq_1'].'</h5>';
            $retUltimoID = $u->retUltimoID('cd_venda_seq_1', 'tb_venda', $_SESSION['cd_empresa'], $_SESSION['cd_filial']);

            //echo '<h5>Ultimo ID: '.$retUltimoID['ultimo_id'].'</h5>';
            //echo '<h5>Próximo ID: '.$retUltimoID['proximo_id'].'</h5>';
...
<?php
  if($_SESSION['cd_venda'] > 0){ 
    $result_financeiro  = $u->movimentoFinanceiro($_SESSION['dt_caixa'], $_SESSION['cd_empresa'],'', $_SESSION['cd_venda']);
    $_SESSION['falta_pagar_venda'] = $result_financeiro['falta_pagar'];
...
<!--<form method="post" action="0_finalizar.php">
  <button class="btn btn-success w-100"
    ...
    Finalizar Venda
          ...
          <?php 
            //echo '<h1>'.$_SESSION['falta_pagar_venda'].'</h1>';
...
function confirmarReimpressao(cdVenda) {
    //if (confirm('Deseja reimprimir esta venda?')) {
        window.location.href = '0_reimprimir.php?cd_venda=' + cdVenda;
    //}
}
