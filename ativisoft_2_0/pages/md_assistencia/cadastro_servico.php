                      ...
                      /*echo '
                        <script>
                          if (window.history.replaceState) {
                            ...
                            console.log("Histórico de formulário limpo");
                          }
                        </script>
                      ';
*/
                  ...
                  if(isset($_POST['lancarOrcamento'])) {
                    $valor_br = $_POST['vcusto_orcamento']; 
                    $valor = str_replace(',', '.', $valor_br); 
                    $valor_float = floatval($valor); 

                    $result_orcamento = $u->cadOrcamento('AVULSO', $_POST['cd_cliente'],$_SESSION['cd_empresa'], $_SESSION['cd_filial'], $_POST['cd_servico'], $_POST['titulo_orcamento'],  number_format($valor_float, 2, '.', '.'));
                  ...
                  if(isset($_POST['listaremover_orcamento'])) {//DELETE FROM `tb_orcamento_servico` WHERE `tb_orcamento_servico`.`cd_orcamento` = 198
                    ...
                    if (($orcamento_servico - $valor_orcamento) >= $vpag_servico) {
                      ...
                      /*$updateVtotalServico = "UPDATE tb_servico SET
                        ...
                        WHERE cd_servico = ".$_SESSION['cd_servico']."";
                        $_POST['listaid_orcamento']*/
                      $updateVtotalServico = "
                        UPDATE tb_servico
                        SET orcamento_servico = orcamento_servico - (
                            SELECT vcusto_orcamento
                            FROM tb_orcamento_servico
                            WHERE cd_orcamento = ".(int)$_POST['listaid_orcamento']."
                            LIMIT 1
                        )
                        WHERE cd_servico = ".(int)$_SESSION['cd_servico']."
                      ";
/*
                      UPDATE tb_servico
                        SET orcamento_servico = orcamento_servico - (
                            SELECT vpag_orcamento
                            FROM tb_orcamento_servico
                            WHERE cd_orcamento = ".(int)$_POST['listaid_orcamento']."
                            LIMIT 1
                        )
                      ...
                      if(mysqli_query($conn, $updateVtotalServico)){
                        echo "<script>window.alert('orcamento_servico atualizado!');</script>";  
                      }else{
                        echo "<script>window.alert('Erro!');</script>";  
                      }
                      ...
                      echo "<script>window.alert('Valor pago não pode ser maior que o total do Orcamento!');</script>";  
                    }
                  }

                  $modoEdicao = false;
$orcamentoEditar = null;

if (isset($_POST['listaabrir_edicao'])) {
    $modoEdicao = true;
    $orcamentoEditar = (int) $_POST['listaid_orcamento'];
}


if (isset($_POST['listaedita_orcamento'])) {

    $cd_orcamento = (int) $_POST['listaid_orcamento'];
    $novoTitulo   = mysqli_real_escape_string($conn, $_POST['listatitulo_orcamento']);
    $novoValor    = floatval(str_replace(',', '.', $_POST['listavalor_orcamento']));

    // valor antigo
    $sqlOld = "SELECT vcusto_orcamento, cd_servico
               FROM tb_orcamento_servico
               WHERE cd_orcamento = $cd_orcamento";
    $resOld = mysqli_query($conn, $sqlOld);
    $old    = mysqli_fetch_assoc($resOld);

    $valorAntigo = floatval($old['vcusto_orcamento']);
    $cd_servico  = (int) $old['cd_servico'];

    // ajusta total do serviço
    mysqli_query($conn, "
        UPDATE tb_servico
        SET orcamento_servico = orcamento_servico - $valorAntigo + $novoValor
    ...
    echo "<script>alert('Orçamento atualizado com sucesso');</script>";
    echo '<script>location.href="'.$_SESSION['dominio'].'pages/md_assistencia/cadastro_servico.php";</script>';
}
                  ...
                  if (isset($_POST['confirmacao'])) {
                    ...
                    if($retorno['status'] == 'OK'){
                      ...
                      if($_SESSION['cd_servico'] > 0){
                        $result_servico     = $u->conServico($_SESSION['cd_servico'], $_SESSION['cd_empresa'], true);
                        $result_cliente     = $u->conPessoa('cliente', 'codigo', $result_servico['cd_cliente']);
                        $result_orcamento   = $u->listOrcamentoServico($result_servico['cd_servico'], $_SESSION['cd_empresa'], true, true, $modoEdicao, $orcamentoEditar);
