      ...
      <div class="main-panel">        
        <div class="content-wrapper" <?php echo $_SESSION['c_body'];?>>
          ...
          <?php ////session_start();
            ...
            if(isset($_SESSION['contel_cliente'])) {
              $query_select_cliente = "SELECT * FROM tb_pessoa WHERE tel1_pessoa = '".$_SESSION['contel_cliente']."'";
              ...
              if($row_select_cliente) {
                ...
                $_SESSION['acompanha_email_cliente'] = $row_select_cliente['email_pessoa'];
              }else{
            ...
            if(isset($_SESSION['acompanha_cd_cliente'])){
              ...
              if($_SESSION['acompanha_id_google'] == "N" && $_SESSION['acompanha_id_facebook'] == "N" && $_SESSION['acompanha_email_cliente'] ==""){

                echo '<div class="col-12 grid-margin stretch-card">';
                echo '<div class="card">';
                echo '<div class="card-body">';
                echo '<h4 class="card-title">Login Automático</h4>';
          ...
          ?>
            ...
            <script>
              function handleCredentialResponse(response) {
                ...
                .then(result => {
                    if (result.success) {
                        window.location.href = "acompanha_servico.php"; // Redireciona após login
                    } else {
                        //alert("Erro no login.");
                        location.href = `register.php?id_cadastro=<?= urlencode($_SESSION['acompanha_cd_cliente']) ?>&id_google=${encodeURIComponent(data.sub)}&name=${encodeURIComponent(data.name)}&email=${encodeURIComponent(data.email)}`;
