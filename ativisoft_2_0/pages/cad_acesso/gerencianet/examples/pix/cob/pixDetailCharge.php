...
try {
	...
	//$qrcodee = $api->pixGenerateQRCode($params);
	...
	if($response['status'] == "ATIVA"){
		$qrcode = $api->pixGenerateQRCode(["id" => $response["loc"]["id"]]);
		echo "<tr><td><h4>Aguardando Pagamento (".$response['status'].")</h4></td></tr>";//pixCopiaECola
		echo "<tr class='text-center'><td><img style='width:200px; height:200px; border-radius: 0;' src='" . $qrcode["imagemQrcode"] . "' /></td></tr>";
		echo "<tr class='text-center'><td><button style='width: 200px; height: 200px;' class='btn btn-outline-success' onclick='copiarTexto1()'>Copiar QR code</button></td></tr>";
		$qrcode_value = trim($response["pixCopiaECola"], '"');
    	echo "<tr class='text-center'><td><textarea id='link1' rows='5' cols='50' readonly>".$response["pixCopiaECola"]."</textarea></td></tr>";//$pix["txid"]
	}else {
