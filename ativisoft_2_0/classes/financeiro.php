            ...
            if($tipo == 'R'){
                ...
                try {
                    ...
                    if ($result_servico->num_rows > 0) {
                        $row = $result_servico->fetch_assoc(); // Pega a linha como array associativo
                        
                        $conn->commit(); // ❗ Se quiser salvar de verdade, troque por $conn->commit();
                        return [
                            'status'        => 'sucesso',
                            'servico_vpag'  => $row['vpag_servico']
                        ];

                    } else {
                        $conn->rollback();
                        return [
                            'status'        => 'Serviço ('.$cd_servico.') não encontrado',
                            'servico_vpag'  => '0'
                        ];
                    }
