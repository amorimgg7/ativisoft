...
class Usuario  
{
    ...
    public function cadEmpresa($logo_empresa, $rsocial_empresa, $nfantasia_empresa, $cnpj_empresa, $iestadual_empresa, $imunicipal_empresa, $dtabertura_empresa, $obs_empresa, $tel1_empresa, $obs_tel1_empresa, $tel2_empresa, $obs_tel2_empresa, $email_empresa)
    {
        global $pdo;
    ...
    public function cadColab($foto_colab, $pnome_colab, $snome_colab, $cpf_colab, $dtnasc_colab, $sexo_colab, $obs_colab, $tel1_colab, $obs_tel1_colab, $email_colab, $cd_funcao, $senha_colab)
    //public function cadPessoal1($pnome_pessoal, $snome_pessoal, $cpf_pessoal, $rg_pessoal, $cnh_pessoal, $carttrabalho_pessoal, $pis_pessoal, $dtnasc_pessoal, $sexo_pessoal, $ecivil_pessoal, $tel1_pessoal, $obs_tel1_pessoal, $tel2_pessoal, $obs_tel2_pessoal, $email_pessoal, $senha_pessoal)
    {
        global $pdo;
        //verificar se ja está cadastrado
        $sql = $pdo->prepare("SELECT cd_colab FROM tb_colab WHERE cpf_colab = :cpf");
        $sql->bindValue(":cpf", $cpf_colab);
        $sql->execute();
        if($sql->rowCount() > 0)
        {
        ...
        {//não cadastrado, cadastrando agora
            $sql = $pdo->prepare("INSERT INTO tb_colab(foto_colab, pnome_colab, snome_colab, cpf_colab, dtnasc_colab, sexo_colab, obs_colab, tel1_colab, obs_tel1_colab, email_colab, senha_colab) VALUES (:ftc, :pnc, :snc, :cfc, :dnc, :sxc, :obc, :t1c, :o1c, :emc, :sec)");
            $sql->bindValue(":ftc", $foto_colab);
            $sql->bindValue(":pnc", $pnome_colab);
            $sql->bindValue(":snc", $snome_colab);
            $sql->bindValue(":cfc", $cpf_colab);
            $sql->bindValue(":dnc", $dtnasc_colab);
            $sql->bindValue(":sxc", $sexo_colab);
            $sql->bindValue(":obc", $obs_colab);
            $sql->bindValue(":t1c", $tel1_colab);
            $sql->bindValue(":o1c", $obs_tel1_colab);
            $sql->bindValue(":emc", $email_colab);
            $sql->bindValue(":sec", $senha_colab);//md5($senha_morador);
            //$sql->bindValue(":sep", md5($senha_pessoal));
            $sql->execute();
            //echo '<script>location.href="login.php";</script>';

            $sql = $pdo->prepare("SELECT * FROM tb_colab WHERE cpf_colab = :cf");
            $sql->bindValue(":cf", $cpf_colab);
            $sql->execute();
            if($sql->rowCount() > 0)
            {
                ...
                if($sql1->rowCount() == 0){
                    $sql2 = $pdo->prepare("INSERT INTO rel_user(cd_seg, cd_colab, cd_estilo, cd_funcao, cd_empresa, cd_status) VALUES (1, :cdp, 1, :cdf, 1, 1)");
                    $sql2->bindValue(":cdp", $_SESSION['cd_colab']);
                    $sql2->bindValue(":cdf", $cd_funcao);
                    $sql2->execute();
                }
    ...
    public function logar($cnpj_empresa, $email_colab, $senha_colab) 
    {
        ...
        //$sql->bindValue(":emc", $email_colab);
        //$sql->bindValue(":sn", $senha_colab);//md5($senha_morador);
        //$sql->execute();
        ...
        if($sql->rowCount() > 0)
        {
            ...
            //$_SESSION['foto_usuario'] = $morador['foto_usuario'];
