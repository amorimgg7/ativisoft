...
if($_POST['marcartitulo_atividade'] == 'D') {// SQL ENTREGA / DEVOLVE AO CLIENTE
  ...
  if(!mysqli_query($conn, $query)){
    echo "<script>window.alert('Erro na atividade!');</script>";
  }else{
    echo "<script>window.alert('sucesso 1');</script>";
  ...
  if(!mysqli_query($conn, $query)){
    //falhou
    echo "<script>window.alert('Erro no Serviço!');</script>";
  }else{
    echo "<script>window.alert('sucesso 2');</script>";
  }
